"""Tests for scripts/m4_transaction_cost_sweep.py — spec_2026_05_26_001."""
...
from scripts.m4_transaction_cost_sweep import (
    COST_BPS,
    LEDGER_PATH,
    SUMMARY_PATH,
    compute_row,
...
def test_zero_bps_reproduces_baseline():
    """At cost_bps=0, pf/expectancy/win_rate/N match m4_5yr_summary_D6.json within tolerance."""
...
def test_input_ledger_not_mutated():
    """hashlib digest of m4_5yr_trades_D6.csv is byte-identical before and after a full sweep."""
...
def test_n_constant_across_grid():
    """N == 44 at every cost level — cost changes trade sign, never trade count."""
    rows = run_sweep()
    assert [r["cost_bps"] for r in rows] == COST_BPS
    for row in rows:
        assert row["n"] == 44, f"N changed at {row['cost_bps']} bps: {row['n']}"


def test_pf_undefined_handled():
    """Zero losing trades → pf='inf', pf_undefined=True, no ZeroDivisionError."""
